...
优化器的作用：遍历生成的模板 AST 树并检测纯静态的子树，即永远不需要更改的 DOM 部分
一旦我们检测到这些子树，我们就可以：
...
```
## markStatic
标记静态节点，为AST节点添加```static```属性。**需要注意，只要有一个子节点不是静态节点，那么该节点也不是静态节点**
```js
// 标记静态节点
...
2. 标签上存在v-pre指令
3.  
    - 没有使用v-xxx等一系列指令，包括简写
    - && 没有使用v-if和v-for和v-else
    - && 不是vue的内置组件，slot和component标签
    - && 是平台保留标签，即不是组件标签
    - && 所有父级节点中不能为具有v-for指令的template标签
    - && 该节点的AST所有属性，都能在这些属性列表中找到```'type,tag,attrsList,attrsMap,plain,parent,children,attrs,start,end,rawAttrsMap,staticClass,staticStyle'```
```js
/*
满足下列条件中的一项则为静态节点
1.普通的文本节点
...
```
## markStaticRoots
标记静态根节点，**只有type为1的标签，即标签节点，才能标记为根节点**   
下列条件都满足则标记为静态根节点
1. 节点是静态节点
2. 节点有子节点
3. 且子节点不能只有一个普通的文本节点
