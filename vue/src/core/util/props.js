...
export function validateProp (
  ...
  if (booleanIndex > -1) {// 大于-1，则说明Boolean类型是在type中是存在的
    ...
    type类型包含Boolean
        ...
        2.msg,传入为空字符串，分两种（1）type数组中String在Boolean的前面，则赋值为空字符串，不变（2）type数组中Boolean在String的前面或只存在Boolean，赋值为true
        3.comp-data，传入的值与key值相等,分两种（1）type数组中String在Boolean的前面，则赋值为"comp-data"，不变（2）type数组中Boolean在String的前面或只存在Boolean，赋值为true
